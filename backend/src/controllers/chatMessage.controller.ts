...
import { generateResponse, validateMessage } from '../services/langchain.service';
 ...
 */
async function sendMessage(req: AuthRequest, res: Response): Promise<Response> {
    try {
        ...
        try {
            ...
            if (chatDoc && chatDoc.isTemp && msgCount === 2) {
                ...
                const newTitle = await generateTitleFromText(message);
